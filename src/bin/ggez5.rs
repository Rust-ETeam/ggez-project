...
use ggez::graphics::{Image, Color, DrawParam, Text, draw, clear, present};
...
impl EventHandler for GGEZ {
    fn update(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        if self.player_grab_state == 1 {
            ...
            if self.player_grab_time <= 0.0 {
                ...
                if self.player_score == 3 || self.opponent_score == 3 {
                    self.game_end = true;
                    return Ok(());
                }
                self.send_player_data();
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        let opponent_score_draw_params = DrawParam::new()
            ...
            .scale([2.0, 2.0])
            .color(Color::WHITE);
        draw(ctx, &Text::new(format!("{}", self.opponent_score)), opponent_score_draw_params)?;
        ...
        let player_score_draw_params = DrawParam::new()
            ...
            .scale([3.0, 3.0])
            .color(Color::WHITE);
        draw(ctx, &Text::new(format!("{}", self.player_score)), player_score_draw_params)?;
        ...
        if self.game_end {
            ...
            if self.player_score == 3 {
                ...
                draw(ctx, &Text::new("Opponent Win!\nPress ESC"), game_end_draw_params)?;
            }
            present(ctx)?;
            return Ok(());
        }
